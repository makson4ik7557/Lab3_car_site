"""
Dashboard views для аналітичних графіків з використанням Plotly
Отримує дані через Repository (правильна архітектура: DB → Repository → UI)
"""
...
def plotly_dashboard(request):
    ...
    АРХІТЕКТУРА: DB → Repository → API → UI (Dashboard)
    Всі дані отримуються через REST API endpoints, а не напряму з Repository!

    Використовує всі 6 агрегованих запитів для візуалізації:
    ...
    # Отримуємо дані напряму з Repository
    repo_service = RepositoryService()
    sales_data = repo_service.analytics.get_sales_by_employee(min_sales_count=min_sales)
    logger.info(f"Графік 1 - отримано {len(sales_data)} записів з Repository")

    # Конвертуємо Decimal в float ДО створення DataFrame
    for record in sales_data:
        for key in ['total_revenue', 'average_sale_price', 'max_sale', 'min_sale']:
    ...
    brand_data = repo_service.analytics.get_profit_by_car_brand(min_cars_sold=min_cars)
    logger.info(f"Графік 2 - отримано {len(brand_data)} записів з Repository")

    # Конвертуємо Decimal в float ДО створення DataFrame
    for record in brand_data:
        for key in ['total_revenue', 'average_price', 'highest_sale']:
            if key in record and isinstance(record[key], Decimal):
                record[key] = float(record[key])
    ...
    if not df_brands.empty:
        ...
        fig2 = go.Figure(data=[go.Pie(
            labels=df_brands['car__make'].tolist(),  # Конвертуємо в list!
            values=df_brands['total_revenue'].tolist(),  # Конвертуємо в list!
            hole=0.3,
            marker=dict(colors=px.colors.qualitative.Set3),
            textinfo='label+percent',
            hovertemplate='<b>%{label}</b><br>Дохід: $%{value:,.2f}<br>Частка: %{percent}<br>Продано авто: %{customdata}<extra></extra>',
            customdata=df_brands['cars_sold'].tolist()  # Конвертуємо в list!
        )])
    ...
    transaction_data = repo_service.analytics.get_transaction_dynamics_by_dealer(min_amount=min_amount)
    logger.info(f"Графік 3 - отримано {len(transaction_data)} записів з Repository")

    # Конвертуємо Decimal в float ДО створення DataFrame
    for record in transaction_data:
        for key in ['total_amount', 'average_amount', 'latest_balance']:
            if key in record and isinstance(record[key], Decimal):
                record[key] = float(record[key])
    ...
    if not df_transactions.empty:
        ...
        for trans_type in df_transactions['transaction_type'].unique():
            ...
            fig3.add_trace(go.Scatter(
                x=df_type['dealer__username'].tolist(),  # Конвертуємо в list!
                y=df_type['total_amount'].tolist(),  # Конвертуємо в list!
                mode='lines+markers',
                name=trans_type,
                line=dict(width=2),
                marker=dict(size=8),
                hovertemplate='<b>%{x}</b><br>Тип: ' + trans_type + '<br>Сума: $%{y:,.2f}<br>Кількість: %{customdata}<extra></extra>',
                customdata=df_type['transaction_count'].tolist()  # Конвертуємо в list!
            ))
    ...
    customers_data = repo_service.analytics.get_top_customers_by_spending(limit=top_customers_limit)
    logger.info(f"Графік 4 - отримано {len(customers_data)} записів з Repository")

    # Конвертуємо Decimal в float ДО створення DataFrame
    for record in customers_data:
        for key in ['total_spent', 'average_purchase', 'most_expensive_purchase']:
            if key in record and isinstance(record[key], Decimal):
                record[key] = float(record[key])
    ...
    if not df_customers.empty:
        ...
        fig4 = go.Figure(data=go.Scatter(
            x=df_customers['cars_purchased'].tolist(),  # Конвертуємо в list!
            y=df_customers['total_spent'].tolist(),  # Конвертуємо в list!
            mode='markers+text',
            marker=dict(
                size=(df_customers['total_spent'] / df_customers['total_spent'].max() * 50).tolist(),  # Конвертуємо в list!
                color=df_customers['total_spent'].tolist(),  # Конвертуємо в list!
                colorscale='Viridis',
                showscale=True,
                colorbar=dict(title="Витрачено ($)")
            ),
            text=df_customers['full_name'].tolist(),  # Конвертуємо в list!
            textposition='top center',
            hovertemplate='<b>%{text}</b><br>Куплено авто: %{x}<br>Витрачено: $%{y:,.2f}<br>Середня ціна: $%{customdata:,.2f}<extra></extra>',
            customdata=df_customers['average_purchase'].tolist()  # Конвертуємо в list!
        ))
    ...
    car_stats_data = repo_service.analytics.get_car_price_statistics_by_year(min_cars_count=min_cars_year)
    logger.info(f"Графік 5 - отримано {len(car_stats_data)} записів з Repository")

    # Конвертуємо Decimal в float ДО створення DataFrame
    for record in car_stats_data:
        for key in ['average_price', 'max_price', 'min_price']:
            if key in record and isinstance(record[key], Decimal):
                record[key] = float(record[key])
    ...
    if not df_car_stats.empty:
        ...
        fig5.add_trace(go.Bar(
            x=df_car_stats['year'].tolist(),  # Конвертуємо в list!
            y=df_car_stats['average_price'].tolist(),  # Конвертуємо в list!
            name='Середня ціна',
            marker_color='rgb(158, 202, 225)',
            hovertemplate='<b>Рік: %{x}</b><br>Середня ціна: $%{y:,.2f}<br>Авто: %{customdata}<extra></extra>',
            customdata=df_car_stats['cars_count'].tolist()  # Конвертуємо в list!
        ))

        fig5.add_trace(go.Scatter(
            x=df_car_stats['year'].tolist(),  # Конвертуємо в list!
            y=df_car_stats['max_price'].tolist(),  # Конвертуємо в list!
            mode='lines+markers',
        ...
        fig5.add_trace(go.Scatter(
            x=df_car_stats['year'].tolist(),  # Конвертуємо в list!
            y=df_car_stats['min_price'].tolist(),  # Конвертуємо в list!
            mode='lines+markers',
    ...
    dealer_balance_data = repo_service.analytics.get_dealer_balance_summary(min_transactions=min_transactions)
    logger.info(f"Графік 6 - отримано {len(dealer_balance_data)} записів з Repository")
    logger.info(f"Графік 6 - сирі дані: {dealer_balance_data}")

    # Конвертуємо Decimal в float та обробляємо None ДО створення DataFrame
    for record in dealer_balance_data:
        for key in ['current_balance', 'total_buy_amount', 'total_sell_amount', 'average_transaction']:
            if key in record:
                if record[key] is None:
                    record[key] = 0.0
                elif isinstance(record[key], Decimal):
                    record[key] = float(record[key])
    ...
    if not df_dealer_balance.empty:
        ...
        fig6.add_trace(go.Bar(
            x=df_dealer_balance['dealer__username'].tolist(),  # Конвертуємо в list!
            y=df_dealer_balance['buy_transactions'].tolist(),  # Конвертуємо в list!
            name='Купівлі',
            marker_color='rgb(255, 127, 80)',
            hovertemplate='<b>%{x}</b><br>Купівель: %{y}<br>Сума: $%{customdata:,.2f}<extra></extra>',
            customdata=df_dealer_balance['total_buy_amount'].tolist()  # Конвертуємо в list!
        ))

        fig6.add_trace(go.Bar(
            x=df_dealer_balance['dealer__username'].tolist(),  # Конвертуємо в list!
            y=df_dealer_balance['sell_transactions'].tolist(),  # Конвертуємо в list!
            name='Продажі',
            marker_color='rgb(144, 238, 144)',
            hovertemplate='<b>%{x}</b><br>Продажів: %{y}<br>Сума: $%{customdata:,.2f}<extra></extra>',
            customdata=df_dealer_balance['total_sell_amount'].tolist()  # Конвертуємо в list!
        ))

        fig6.add_trace(go.Bar(
            x=df_dealer_balance['dealer__username'].tolist(),  # Конвертуємо в list!
            y=df_dealer_balance['modify_transactions'].tolist(),  # Конвертуємо в list!
            name='Модифікації',
        ...
        fig6.add_trace(go.Scatter(
            x=df_dealer_balance['dealer__username'].tolist(),  # Конвертуємо в list!
            y=df_dealer_balance['current_balance'].tolist(),  # Конвертуємо в list!
            mode='lines+markers',
